// 接口：用于约束函数、对象、类的结构和类型，是一种必须遵守的代码协作的契约，不可更改。
// 示例场景：从后端获取数据并渲染到前端页面，定义接口如下：
...
// 原因：TS 采用了一种“鸭式变形”法，这是一种动态语言的类型风格
// （一只鸟如果看起来像鸭子、游起来像鸭子、叫起来像鸭子，那么这只鸟就可以被认为是鸭子）
// 只要传入的对象满足接口定义的必要条件，即使传入多余的字段，也是允许的，也可以通过类型检查。

// 但若直接传入对象字面量作渲染函数的参数，则会报错：
// render({
//     data: [
//         {id: 1, name: 'A', sex: 'male'},    // Error: 不能将类型“{ id: number; name: string; sex: string; }”分配给类型“List”。
